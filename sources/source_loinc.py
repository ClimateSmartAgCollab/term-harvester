"""LOINC source helpers for term_harvester.py.
...
def fill_loinc_source_metadata(output_path, key, config_file=MENU_CONFIG):
    ...
    if not source.get("name"):
        ...
        derived_name = "LOINC_" + (to_camel_case(raw_name) if len(parts) > 1 else raw_name)
...
def process_loinc_table_source(key, source, config_file=MENU_CONFIG):
    ...
    for row in rows:
        ...
        enum_key = "LOINC_" + (to_camel_case(name) if len(parts) > 1 else name)
...
def match_loinc_table(url, tmp_path, config_file=MENU_CONFIG):
    ...
    key = "LOINC_" + to_camel_case(url_stem_noext)
