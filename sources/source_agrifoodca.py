"""AgriFoodCA picklist source helpers for term_harvester.py.
...
def _filename_to_key(filename):
    ...
    Examples::
        ...
        'soil_drainage.csv'             -> 'AgriFoodCA_SoilDrainage'
        'canadian_provinces.csv'        -> 'AgriFoodCA_CanadianProvinces'
        'education_level_stats_can.csv' -> 'AgriFoodCA_EducationLevelStatsCan'
    """
    stem = re.sub(r'\.csv$', '', filename, flags=re.IGNORECASE).strip()
    return 'AgriFoodCA_' + to_pascal_case_key(stem)
...
def match_agrifood_dir(url, config_file=MENU_CONFIG):
    ...
    # /tree/{branch}/{dir} → AgriFoodCA_ + PascalCase(dir); repo root → AgriFoodCA_Picklists.
    dir_m = re.search(r'/tree/[^/]+/([^/?#]+)', url, re.IGNORECASE)
    dir_name    = dir_m.group(1) if dir_m else 'picklists'
    combined_key = 'AgriFoodCA_' + to_pascal_case_key(dir_name)
