"""NAPCSCanada source helpers for term_harvester.py.
...
def match_napcs_csv(url, tmp_path, config_file=MENU_CONFIG, downloaded_filename=""):
    ...
    if variant_m:
        ...
        key = f"NAPCSCanada_{to_pascal_case_key(variant_name)}_{year}" if year else f"NAPCSCanada_{to_pascal_case_key(variant_name)}"
        ...
        key = f"NAPCSCanada_{year}" if year else "NAPCSCanada"
